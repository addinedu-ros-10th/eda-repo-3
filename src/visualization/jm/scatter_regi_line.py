...
# 중간 그룹 (회색)
sns.scatterplot(data=middle, x='hospital_count', y='total_registered', label='기타 지역', color='gray')

# 상위 5개 (파란색)
sns.scatterplot(data=top5, x='hospital_count', y='total_registered', label='병원 수 상위 5개', color='blue')

# 하위 5개 (빨간색)
sns.scatterplot(data=bottom5, x='hospital_count', y='total_registered', label='병원 수 하위 5개', color='red')

# 회귀선 (전체 데이터 기준)
sns.regplot(data=df, x='hospital_count', y='total_registered',
            scatter=False, color='black', line_kws={"linestyle": "dashed"}, label='회귀선')

# 라벨 추가
for i in range(df.shape[0]):
    plt.text(x=df.loc[i, 'hospital_count'] + 0.5,
             y=df.loc[i, 'total_registered'],
             s=df.loc[i, 'district'],
             fontsize=8)

# 제목 및 축
plt.title('병원 수 vs 반려동물 등록수', fontsize=14)
